...
if [ "$(id -u)" -ne 0 ]; then
    echo "This script must be run as root. Please use doas or run as root user."
    exit 1
fi
...
pkg_add -U wget jdk-21.0.2.13.1v0
...
{
    echo 'SERVICE_SCRIPT="/etc/rc.d/minecraft"'
    ...
    echo "PLEDGE_COMMAND=\"pledge 'stdio rpath wpath cpath tmux proc exec unix'\""
    echo "UNVEIL_COMMAND=\"unveil '/usr/local/bin/java' r '/var/minecraft_server' rw '$(command -v tmux)' r '/dev/urandom' r\""
    echo "MINECRAFT_COMMAND=\"exec \$JAVA_PATH -Xmx\$MEMORY_ALLOCATION -Xms\$INITIAL_MEMORY -jar \$MINECRAFT_JAR nogui\""
    echo "START_COMMAND=\"\$RESOURCE_LIMIT_COMMAND && \$PLEDGE_COMMAND && \$UNVEIL_COMMAND && \$MINECRAFT_COMMAND\""
} >>"$CONFIG_FILE"
...
if ! id -u "$MINECRAFT_USER" >/dev/null 2>&1; then
    ...
    useradd -m -d "$MINECRAFT_DIR" -s /bin/sh -c "Minecraft Server User" "$MINECRAFT_USER"
else
...
if ! getent group "$MINECRAFT_GROUP" >/dev/null 2>&1; then
    ...
    usermod -G "$MINECRAFT_GROUP" "$MINECRAFT_USER"
else
...
if ! rcctl check minecraft | grep -q "Minecraft server is running"; then
    echo "\$(date): Minecraft server is down. Restarting..."
    rcctl start minecraft
...
echo "You can start the Minecraft server with: doas rcctl start minecraft"
